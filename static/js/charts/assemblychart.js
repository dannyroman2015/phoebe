...
const drawAssemblyVTChart = (data, plandata, inventorydata, target) => {
  ...
  const color = d3.scaleOrdinal()
    .domain(["X1-brand", "X1-rh", "X2-brand", "X2-rh", "X1-white", "X2-white", "brand", "rh", "X7-brand", "X7-rh", "X3-brand", "X3-rh"])
    .range(["#DFC6A2", "#A5A0DE", "#DFC6A2", "#A5A0DE", "#D1D1D1", "#D1D1D1", "#DFC6A2", "#A5A0DE", "#DFC6A2", "#A5A0DE", "#DFC6A2", "#A5A0DE"])
    .unknown("white");
  ...
  innerChart.append("text")
        ...
        .attr("y", y(plandata[plandata.length-1].plan + plandata[plandata.length-2].plan) - 20)
        .attr("fill", "#FA7070")
        .attr("font-size", 14)
        .attr("transform", `rotate(90, ${x(plandata[plandata.length-1].date) + x.bandwidth()/6}, ${y(plandata[plandata.length-1].plan + plandata[plandata.length-2].plan) - 20})`)
